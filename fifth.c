...
void main(){
	...
	#pragma omp parallel
	{
		...
		/* Within the different threads there are certain code that need be executed 
		   by a single thread. In certain cases a thread that is not the master
		   thread can reach the code this construct first. The directive #pragma omp single can be used for this */
